...
class TestCrawlerService:
    ...
    def test_run_with_no_results(self, crawler_service, mock_dependencies):
        ...
        empty_report = ScrapeReport(
            final_stock_count=0, spack_filtered_count=5, results=[]
        )
        mock_dependencies[
            "calendar_scraper"
    ...
    def test_run_with_multiple_years(self, crawler_service, mock_dependencies):
        ...
        mock_dependencies["data_mapper"].to_dataframe.return_value = pd.DataFrame(
            [{"name": "test"}]
        )
    ...
    def test_run_daily_no_results(self, crawler_service, mock_dependencies):
        ...
        empty_report = ScrapeReport(
            final_stock_count=0, spack_filtered_count=3, results=[]
        )
        mock_dependencies[
            "calendar_scraper"
        ].scrape_calendar.return_value = empty_report

        result = crawler_service.run_scheduled(
            start_date=date(2024, 11, 26), days_ahead=0
        )
        assert result == {}
        mock_dependencies["repository"].save.assert_not_called()
