...
class TestConsoleLogger:
    ...
    def logger(self):
        ...
        import uuid
        c_logger = ConsoleLogger(name=f"test_{uuid.uuid4().hex[:8]}")
        # 테스트 환경에서는 caplog가 잡을 수 있게 propagate 임시 허용
        c_logger._logger.propagate = True
        return c_logger

    def test_info_message(self, logger, caplog):
        """info 메서드 테스트"""
        with caplog.at_level(logging.INFO, logger=logger._logger.name):
            logger.info("테스트 정보 메시지")
        assert "테스트 정보 메시지" in caplog.text

    def test_warning_message(self, logger, caplog):
        """warning 메서드 테스트"""
        with caplog.at_level(logging.WARNING, logger=logger._logger.name):
            logger.warning("테스트 경고 메시지")
        assert "테스트 경고 메시지" in caplog.text

    def test_error_message(self, logger, caplog):
        """error 메서드 테스트"""
        with caplog.at_level(logging.ERROR, logger=logger._logger.name):
            logger.error("테스트 에러 메시지")
        assert "테스트 에러 메시지" in caplog.text

    def test_multiple_messages(self, logger, caplog):
        """여러 메시지 연속 출력 테스트"""
        with caplog.at_level(logging.DEBUG, logger=logger._logger.name):
            logger.info("첫 번째")
            logger.warning("두 번째")
            logger.error("세 번째")
