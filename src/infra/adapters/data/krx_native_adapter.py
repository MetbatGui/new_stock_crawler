...
class KrxNativeAdapter(TickerMapperPort, MarketDataProviderPort):
    ...
    def __init__(self, mbr_id: Optional[str] = None, pw: Optional[str] = None):
        ...
        self._REF = (
            f"{self.BASE_URL}/contents/MDC/MDI/mdiLoader/index.cmd?menuId=MDC0201"
        )
    ...
    def _login(self) -> bool:
        ...
        if not self.mbr_id or not self.pw:
            logger.error(
                "[KRX] 로그인 계정 정보(KRX_USERNAME/PASSWORD)가 설정되지 않았습니다."
            )
            return False
    ...
    def _fetch_all_markets(self, target_date_str: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if items:
                ...
                logger.info(
                    f"      [KRX] {target_date_str} 전종목 시세 캐싱 완료 ({len(items)}건)"
                )
            return items
        except Exception:
            return []
