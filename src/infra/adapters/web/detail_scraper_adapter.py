...
class DetailScraperAdapter(DetailScraperPort):
    ...
    def _scrape_single(self, page: Page, name: str, href: str) -> Optional[StockInfo]:
        ...
        try:
            ...
            if self.logger:
                self.logger.error(f"[{name}] 스크래핑 실패 ({href}): {e!r}")
            return None
    ...
    def _parse_schedule_info(self, page: Page) -> dict:
        ...
        return {
            ...
            "competition_rate": text_parsers.format_competition_rate(
    ...
    def _create_stock_info(
        ...
        return StockInfo(
            ...
            revenue=text_parsers.parse_to_int(
                company_info["revenue"], f"{name} - revenue"
            ),
            profit_pre_tax=text_parsers.parse_to_int(
                company_info["profit_pre_tax"], f"{name} - profit_pre_tax"
            ),
            net_profit=text_parsers.parse_to_int(
                company_info["net_profit"], f"{name} - net_profit"
            ),
            capital=text_parsers.parse_to_int(
                company_info["capital"], f"{name} - capital"
            ),
            total_shares=text_parsers.parse_to_int(
                offering_info["total_shares"], f"{name} - total_shares"
            ),
            par_value=text_parsers.parse_to_int(
                offering_info["par_value"], f"{name} - par_value"
            ),
            desired_price_range=offering_info["desired_price"],
            confirmed_price=text_parsers.parse_to_int(
                offering_info["confirmed_price"], f"{name} - confirmed_price"
            ),
            offering_amount=text_parsers.parse_to_int(
                offering_info["offering_amount"], f"{name} - offering_amount"
            ),
            ...
            emp_shares=text_parsers.parse_to_int(
                schedule_info["emp_shares"], f"{name} - emp_shares"
            ),
            inst_shares=text_parsers.parse_to_int(
                schedule_info["inst_shares"], f"{name} - inst_shares"
            ),
            retail_shares=text_parsers.parse_to_int(
                schedule_info["retail_shares"], f"{name} - retail_shares"
            ),
    ...
    def _calculate_colspan_range(
        self, grid: List[List[str]], row_idx: int, col_idx: int
    ) -> int:
        """colspan 범위 계산"""
        colspan_end = col_idx
        ...
        while (
            colspan_end < len(grid[row_idx]) - 1
            and grid[row_idx][colspan_end + 1] == cell_value
        ):
            colspan_end += 1
        return colspan_end
